...
public class Sedav22Adapter implements Sedav2Adapter {
    ...
    private static final NamespacePrefixMapper namespaceMapper = new NamespacePrefixMapper() {
        public String getPreferredPrefix(String namespaceUri, String suggestion, boolean requirePrefix) {
            return !requirePrefix && "fr:gouv:culture:archivesdefrance:seda:v2".equals(namespaceUri) ? "" : "ns";
        }
    };

    static {
        // Provide flattened schema in resource
        try (InputStream is1 = SipUtils.resourceAsStream("seda-vitam-2.2-full.xsd"); InputStream is2 = SipUtils.resourceAsStream("xml.xsd"); InputStream is3 = SipUtils.resourceAsStream("xlink.xsd")) {
            ...
            sedaContext = JAXBContext.newInstance(fr.gouv.culture.archivesdefrance.seda.v22.ObjectFactory.class);
        } catch (IOException | JAXBException | SAXException ex) {
            throw new SipException("Unable to initialize XSD Schemas, JAXBContext and Marshaller", ex);
        }
    }

    private final Sedav2Config config;

    public Sedav22Adapter(Sedav2Config config) {
        this.config = config;
    }

    @Override
    public void write(ArchiveTransfer archive, Validator validator, Path zipPath) {

        try (FileSystem zipArchive = SipUtils.newZipFileSystem(zipPath)) {
            ArchiveTransferType att = Sedav22Converter.convert(archive, zipArchive, config);
    ...
    public void validate(ArchiveTransfer archive, Validator validator) {
        try {
            ArchiveTransferType att = Sedav22Converter.convert(archive, config);
